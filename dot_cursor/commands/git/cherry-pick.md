...
| Situation | Suggest |
|-----------|---------|
| Need to commit after cherry-pick | → **git/commit**: "Want me to help commit with git/commit?" |
| Cherry-picking for a PR | → **git/pr**: "This is for a PR - want to use git/pr workflow?" |
| User asks about sweeps/training | → **ml/sweep**: "That's training-related - check ml/sweep?" |
