...
## Pre-Launch Git Safety

**Before launching any NEW sweep, check for uncommitted sweep-critical code.**
...
git status --porcelain | while read status file; do
  # Skip agent-generated files
  [[ "$file" =~ CLAUDE|ACTIVE_SWEEPS|\.md$ ]] && continue
  # Flag sweep-relevant files
  case "$file" in
    experiments/*|configs/*|src/*|*.py|*.yaml|*.sbatch|requirements.txt|pyproject.toml)
      echo "⚠️  SWEEP-RELEVANT: $file ($status)"
      ;;
  esac
done
```
...
1. **Run the check** before `sbatch` for new sweeps
2. **If relevant files found:**
   - List them with their change type (modified/new/deleted)
   - Ask: "These files affect sweep reproducibility. Commit before launching?"
3. **If user confirms:** Use `git/commit` or commit with `[sweep] pre-launch checkpoint`
4. **Record the commit hash** in ACTIVE_SWEEPS.md for recovery

### Confirmation Prompt

Use `ask_question` tool:
- Option 1: "Yes, commit these files"
- Option 2: "No, launch anyway (I understand the risk)"
- Option 3: "Cancel - let me review first"
...
See `~/.cursor/rules/always.md` for patterns. Sweep-specific examples:

| Situation | Example |
|-----------|---------|
| Preempted jobs found | "Should I resume these? I can generate the commands." |
| Short runs flagged | "These failed quickly - want me to check their error logs?" |

---

## Related Commands

See `~/.cursor/rules/command-discovery.md`. Key transitions: git/commit (commit config fixes), git/pr (PR for sweep changes), update (broader status).
