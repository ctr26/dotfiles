...
ssh devbox cat ~/.cursor/commands/ml/sweep.md | diff - ~/.cursor/commands/ml/sweep.md
```
...
| Situation | Suggest |
|-----------|---------|
| After syncing, need to commit | → **git/commit**: "Want to commit these synced changes?" |
| Updating commands before sync | → Edit the command files first |
| User asks about sweeps/training | → **ml/sweep**: "That's training-related - different workflow" |
